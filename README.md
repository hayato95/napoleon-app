...
「ブランチは切ったけど、そこから何していいか分からない」が一番詰まりやすいポイント。ここに書いた流れを目安にする。

### AIツールを使う場合(Claude Code / Cursor / Copilot Chatなど共通)

1. **issueの方針をすり合わせる**：コードを書かせる前に、「このissue、何をすればいいか一緒に整理して。まだ実装しないで」とAIに聞く。認識がズレたまま進めると、後で全部やり直しになりやすい
2. **作業する場所を分ける**：mainのフォルダを汚さないように、issue専用の作業ブランチ・作業フォルダを用意してから始める（参考：git worktreeという仕組みを使うと、複数のissueを同時に触っても中身が混ざらない。気になる人はAIに「git worktreeって何？」と聞いてみてください）
3. **1ファイルずつ、「現状→複数案→実装」を繰り返す**：いきなり全部書かせない
   - そのファイルが今何をしているか確認する
   - 何を・なぜ変えるのか説明してもらう
   - やり方が複数あれば、それぞれの良い点・悪い点を比べる
   - 自分で方法を選び、実装してもらって動作確認する
   - 納得してから次のファイルへ進む
4. **書き終わったらレビュー観点を確認する**：「セキュリティ・入力チェック・エラー処理で抜けてるところない？」とAIに聞く
5. **PRの説明文を一緒に作る**：ここまでのやり取り（現状・方針・実装の経緯）を踏まえて説明文を作ってもらい、内容を確認してからPRを出す

ポイントは「AIに実装は任せるが、設計判断・ドメインの妥当性（ナポレオンのルールとして正しいか）は自分で理解してから進める」こと。理解しないまま進めると、後のレビューで詰まる。難しい言葉が出てきたら「中学生にも分かるように説明して」と聞くと、自分の理解度を確認できる。

### AIツールを使わない場合

1. **issueの「期待する挙動」を自分の言葉で言い換えてみる**：言い換えられないなら、まだ理解できていないということ。分からなければ遠慮なくissueにコメントで質問する
2. **いきなり全部作ろうとしない**：期待する挙動を1行ずつのTODOに分解して、上から順に潰す
3. **詰まったら公式ドキュメント→エラーメッセージのコピペ検索→他の人に聞く、の順で動く**：エラーメッセージをそのまま検索すると大抵誰かが同じところで詰まっている
4. **既存のコードを真似る**：似た処理が他のFRで既に実装されていれば、そのパターンをそのまま踏襲する（ゼロから設計しない）
5. **小さくコミットする**：1つのコミットに詰め込みすぎない。動くところまでで一旦コミットしておくと、詰まったときに戻せる

### 共通

- 分からないまま進めるより、早めに聞く方が結果的に速い
- 「動いた」で満足せず、なぜ動いたか一言で説明できる状態にしてからコミットする

### 参考：私（小北）のAI開発ブログ

技育CAMPとは別のインターンで実際にAIを使いながら開発した話を書いた記事があるので、興味があれば読んでみてください。

[表示できるだけでは終わらない - 何を作り、何をつくらないか考えた５日間](https://developer.dip-net.co.jp/articles/i-k1pgy41ujv)
